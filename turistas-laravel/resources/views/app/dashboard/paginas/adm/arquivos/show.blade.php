...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="d-flex justify-content-between align-items-start mb-3">
            <div>
                <h4 class="fw-bold py-3 mb-0"><span class="text-muted fw-light">/</span> Detalhe do Arquivo</h4>
    ...
    <i class="bx bx-download"></i> Download
                ...
                @role('admin')
                    <button class="btn btn-sm btn-outline-warning" data-bs-toggle="modal" data-bs-target="#editStatusModal">
 ...
 <!-- Modal para editar status -->
    <div class="modal fade" id="editStatusModal" tabindex="-1" aria-labelledby="editStatusModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form action="{{ route('arquivos2.updateStatus', $arquivo->id) }}" method="POST">
                @csrf
                @method('PATCH')
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editStatusModalLabel">Editar Status do Arquivo</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                    </div>
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="status-{{ $arquivo->id }}" class="form-label">Status</label>
                            <select name="status" id="status-{{ $arquivo->id }}" class="form-select" required>
                                <option value="pendente" {{ $arquivo->status === 'pendente' ? 'selected' : '' }}>Pendente</option>
                                <option value="aprovado" {{ $arquivo->status === 'aprovado' ? 'selected' : '' }}>Aprovado</option>
                                <option value="arquivado" {{ $arquivo->status === 'arquivado' ? 'selected' : '' }}>Arquivado</option>
                            </select>
                        </div>
                    </div>
